...
Machbase Neo LLM Chat 패키지는 Machbase Neo 시계열 데이터베이스와 자연어로 상호작용할 수 있는 LLM 채팅 인터페이스입니다. 대화를 통해 데이터 조회, 대시보드 생성, 분석 리포트 작성, 메뉴얼 문서 탐색이 가능합니다.
...
1. 브라우저에서 Machbase Neo 웹 UI를 엽니다.
2. 왼쪽 메뉴 바에서 **PKG** 탭을 선택합니다.
3. 패키지 목록에서 `neo-pkg-llm-chat`을 찾아 **Install**을 클릭합니다.
4. 패키지가 자동으로 필요한 구성 요소를 다운로드하고, 서비스로 등록한 뒤 시작합니다.

### 제거

1. PKG 탭에서 `neo-pkg-llm-chat`을 선택합니다.
2. **Uninstall**을 클릭하여 패키지와 서비스를 제거합니다.
...
프로바이더 및 연결 설정은 웹 기반 Settings 페이지에서 구성할 수 있습니다. API 키, 모델 선택, Machbase Neo 데이터베이스 연결(호스트, 포트, 사용자, 비밀번호)을 브라우저에서 직접 설정합니다.

## 에이전틱 루프

에이전틱 루프는 핵심 실행 엔진입니다. 사용자가 질문을 보내면 시스템이 먼저 질문 유형을 감지한 뒤, LLM이 도구를 선택하고 실행하는 자율 루프에 진입합니다 (최대 60 스텝).

### 질문 유형 감지

시스템이 자동으로 사용자 질문을 분류하고 해당 모드를 활성화합니다:

- "리포트", "보고서", "report"가 포함된 질문 → **리포트 모드** (사전 정의 HTML 분석 리포트 생성)
- "심층", "다각도", "FFT", "RMS"가 포함된 질문 → **고급 모드** (사전 정의 TQL 차트 템플릿 사용)
- 그 외 분석/대시보드 요청 → **기본 모드** (테이블 기반 차트)
...
저장 전 수행되는 검증:
1. 잘못된 ROLLUP 단위를 확인합니다 (예: `ROLLUP('ms')`는 미지원, 최소 단위는 `sec`).
...
템플릿 확장기가 `{TABLE}`, `{TAG}`, `{UNIT}`, `{TIME_START}`, `{TIME_END}` 플레이스홀더를 현재 분석 컨텍스트의 실제 값으로 대체합니다.
...
`server` 모드로 실행 시 패키지가 제공하는 HTTP API 엔드포인트입니다.
